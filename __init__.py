...
class trackerClass:
	def __init__(self,camIndex,camRes,previewDownsize=1,faceDetectionScale=4,eyeDetectionScale=2,timestampMethod=0):
		self.qTo = billiard.Queue()
		self.qFrom = billiard.Queue()
		self.process = billiard.Process( target=self.loop , args=(self.qTo,self.qFrom,camIndex,camRes,previewDownsize,faceDetectionScale,eyeDetectionScale,timestampMethod) )
	def start(self):
		self.process.start()
	def stop(self):
		self.qTo.put('quit')
		...
		if self.process.is_alive():
			self.process.terminate()
		return None
	def loop(self,qTo,qFrom,camIndex,camRes,previewDownsize,faceDetectionScale,eyeDetectionScale,timestampMethod):
		import numpy
		import cv2
		import scipy.ndimage.filters
		...
		def doHaarDetection(image,faceCascade,eyeLeftCascade,eyeRightCascade,faceDetectionScale,eyeDetectionScale):
			...
			return dotList
		#define a function to do cleanup and exit
		def exit_safely():
			camera.stop()
		...
		while True:
			...
			if not camera.qFrom.empty():
				...
				if len(dotList)==0: #no dots found yet, find using haar detection
					if doDots:
						dotList = doHaarDetection(image,faceCascade,eyeLeftCascade,eyeRightCascade,faceDetectionScale,eyeDetectionScale)
